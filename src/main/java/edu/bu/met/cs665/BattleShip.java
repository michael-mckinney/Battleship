...
public class BattleShip {
	...
	public void playGame() {
		...
		while (true) {
			...
			// let's factor out the shot

			// check shot - game over will return true
	...
	public boolean registerShot(Player player, int[] shot) {
		...
		if (ct.checkEmpty(shot)) {
			// maybe I should change the checkOccupied method and a set_State method to the
			// ControlTower instead
		...
		// update location if it is a hit
			...
			player.getPlayerBoard().boardGrid[shot[0]][shot[1]].getShip().decrementHealth();
			if (gameOver(player)) {
